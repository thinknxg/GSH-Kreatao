...
def get_data(filters):
    ...
    if filters.include_attendance_without_checkins:
        ...
        for d in without_checkins:
            if d.get("shift") in SHIFT_STATUS_OVERRIDE and d.get("status") in ("Absent", "Present"):
                d.status = SHIFT_STATUS_OVERRIDE[d.get("shift")]
...
def update_data(data, filters):
    for d in data:
        ...
        # Override WO/NO shifts — never show as Absent
        if d.get("shift") in SHIFT_STATUS_OVERRIDE and d.get("status") in ("Absent", "Present"):
            d.status = SHIFT_STATUS_OVERRIDE[d.get("shift")]
