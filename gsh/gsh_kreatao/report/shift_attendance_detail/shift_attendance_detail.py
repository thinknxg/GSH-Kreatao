...
def is_night_off(date, employee, company):
    emp_holiday_list = frappe.db.get_value("Employee", employee, "holiday_list")
    if emp_holiday_list:
...
def get_data(filters):
    ...
    for emp in employees:
        for date in dates:
            ...
            # NO ATTENDANCE
            # If shift filter is applied → skip unless it's a WO/Holiday/Night Off day
            if filters.get("shift"):
                dummy_check = frappe._dict({"status": "", "reason": None})
                check_status = resolve_final_status(
                    entry=dummy_check,
                    date=date,
                    employee=emp.name,
                    company=emp.company
                )
                if check_status not in ("WO", "H", "NO"):
                    continue
            ...
            # Determine shift label for dummy rows
            shift_label = ""
            if status == "WO":
                shift_label = "Weekly Off"
            elif status == "NO":
                shift_label = "NIGHT OFF"
            final_data.append(
                frappe._dict(
                    {
                        "employee": emp.name,
...
def get_report_summary(data):
    present_records = half_day_records = absent_records = leave_records = od_records = os_records = wfh_records = holiday_records = weekly_off_records = night_off_records = 0
    ...
    return [
        ...
        {"value": night_off_records, "label": _("Night Off"), "datatype": "Int", "indicator": "#1a1a6e"},
...
def get_query(filters):
    ...
    for filter in filters:
        if filter == "from_date":
            ...
            continue
        elif filter == "include_shift_attendance_without_checkins":
            continue
